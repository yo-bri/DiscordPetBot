...
\* **Active / daycare status is a boolean inside each pet record**  
\* Swapping between active pet and daycare is free with no cooldown  
\* **Equipped pet UUID stored in user profile.json under "current_pet"**  
\* \*\*Stressed/distressed pets cannot be moved to daycare\*\* - they won't go out of fear  
...
\- **Backend values stored in config/backend_values.json for scam detection**  
\- **Trades with value difference >5 prompt scam warning**  
\- **High backend-value items blocked from gift command**  
\- **User must answer a random security question before high-value trade completes**  

# Additional Systems

## Daycare/Inventory System
\- Only one pet can be active at a time  
\- **Daycare status stored as boolean inside each pet record**  
\- **Equipped pet UUID stored in user profile.json under "current_pet"**  
...
\- **Users set ≥3 backup questions on first use**  
\- **Answers stored as bcrypt hashes only**  
\- **Random question asked before high-value trades or gifting rare items**  
\- **Failed answers cancel the trade/gift**  
\- **Ticket reset: open ticket → mod flips flag → user reruns setup**  
...
\- **Single pets.json per user with daycare boolean + current_pet in profile.json**  
...
\- Pet visiting  
\- Gifting mechanics  
\- **Backend-value scam guard + security questions on high-value trades/gifts**  
...
\* \*\*Security questions + backend values protect users from scams\*\* without harming legitimate trades
